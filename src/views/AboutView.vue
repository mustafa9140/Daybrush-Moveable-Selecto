...
export default {
  ...
  methods: {
    ...
    onSelectEnd(e) {
      ...
      let { nMoveable } = this;
      this.selectLinkedObjects(e.selected);
      console.log(nMoveable, e.selected);
      ...
      if (
        ...
        nMoveable.dragStart(e.inputEvent);
      }
    ...
    onSelect(e) {
      ...
      this.nMoveable.dragArea = true;
      this.nMoveable.renderDirections = ["ne", "nw", "se", "sw"];
      this.nMoveable.target = [];
      e;
    },
    selectLinkedObjects(objectList) {
      let { nMoveable } = this;
      let tempLinkedObjects = [];
      let finalSelectedList = [];
      let tempTargetList = objectList;
      tempTargetList = Array.prototype.slice.call(tempTargetList);
      if (tempTargetList && tempTargetList.length) {
        tempTargetList.forEach((obj) => {
          if (obj.dataset.linkid) {
            finalSelectedList.length === 0 && finalSelectedList.push(obj);
            tempLinkedObjects = document.body.querySelectorAll(
              `[data-linkid = "${obj.dataset.linkid}"]`
            );
            tempLinkedObjects = Array.prototype.slice.call(tempLinkedObjects);
            tempLinkedObjects.forEach((target) => {
              let found = finalSelectedList.find((x) => x.id === target.id);
              if (!found) {
                finalSelectedList.push(target);
              }
            });
          } else {
            finalSelectedList.push(obj);
          }
        });
      }
      let tempLock = finalSelectedList.some((target) => target.dataset.locked === "true");
      this.nMoveable.draggable = !tempLock;
      this.nMoveable.rotatable = !tempLock;
      this.nMoveable.resizable = !tempLock;
      this.nMoveable.scalable = !tempLock;
      nMoveable.target = finalSelectedList;
    ...
    handleDragEnd(e) {
      ...
      const frame = this.nframeMap.get(e.target);
      // let object = this.canvasObjects[e.target.dataset.objIndex];
      this.canvasObjects[e.target.dataset.objIndex].translateX = frame.translate[0];
      this.canvasObjects[e.target.dataset.objIndex].translateY = frame.translate[1];
      let angle = parseFloat(frame.rotate);
      this.canvasObjects[e.target.dataset.objIndex].rotate = angle;
      console.log("objIndex: ", e.target.dataset.objIndex);
